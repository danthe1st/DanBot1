...
public class CmdClear implements Command {
	...
	@Override
	public void action(String[] args, final GuildMessageReceivedEvent event) {
		if(args.length>0) {
			int num=getInt(args[0]);
			if(num>1&&num<=100) {
				try {
					final MessageHistory history=new MessageHistory(event.getChannel());
					List<Message>msgs;
					...
					STATIC.errmsg(event.getChannel(),e.getLocalizedMessage());
				} 
